...
class auto_java_DB_class(object):
    ...
    def run_auto_java_DB_func(self):
        ...
        arg3 = "/server/scripts/auto_java_DB_everyday/auto_java_properties/%s.properties" % (self.name)
    ...
    def auto_java_func(DB_name, DB_rename):
        ...
        ###变量式实例化，shell爱用的峰哥
        DB_sql = oracle_run_sql_class(DB_name, 226, "select * from ctl_fc")
...
def main_func(task_group_list):
    # 进程池，同时8个
    pool = multiprocessing.Pool(len(task_group_list))

    # 读配置文件，按照数据库名进行进程池添加
    for line in task_group_list:
        pool.apply_async(pp)
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    aa = oracle_run_sql_class(223)
    result = aa.search_sql_func(search_sql)
    task_group_list = []
    for i in result:
        task_group_list.append(i[0])
    main_func(task_group_list)
...
# # 主函数
# if __name__ == '__main__':
#     daemon = Daemon('/var/run/auto_java_daemon.pid', stdout='/var/log/auto_java_daemon_stdout.log', stderr="/var/log/auto_java_daemon_stderr.log")
#     if len(sys.argv) == 2:
#         if 'start' == sys.argv[1]:
#             daemon.start()
