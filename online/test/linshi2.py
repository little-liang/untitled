import cx_Oracle, datetime, time, sys, calendar, os
...
class Call_StoredProcedure_Class(object):
    ...
    def run_done(self):
        ...
        if res == None:
            return True
        if res[0] == now_date and res[1] == '0':
            print(now_date_time, now_date, '已经跑完了')
            exit()

    ##检查是否任务已经提交过
    def check_command_submit(self):
        now_date = (datetime.datetime.now()).strftime("%Y%m%d")
        ...
        with open(status_file_name, 'r') as f:
            for line in f.readlines():
                ...
                if now_date == str(line_list[1]):
                    print(now_date_time, now_date, status, 'is running, will exit!')
                    exit()


    def write_run_flag(self):
        now_date = (datetime.datetime.now()).strftime("%Y%m%d")
        status = 'R'
        now_date_time = (datetime.datetime.now()).strftime("[%Y-%m-%d-%H:%S]")
    ...
    def flush_run_flag(self):
        with open(status_file_name, "w") as f:
            pass

    # 调用外部依赖标识
    @staticmethod
    def check_before_task_flag_func(*StoredProcedures_Name):
        for line in StoredProcedures_Name:
            print(line)
...
if __name__ == '__main__':
    ...
    if duigong.check_before_task_flag_func():
        ...
        try:
            ...
            print(now_date_time, now_date,'今日调度全部成功')
            ##还差一个 写进去 调度 标识表
            duigong.delete_sql_func()
            duigong.insert_into_sql_func()

            duigong.flush_run_flag()
        except Exception as e:
            print(e)
    else:
        print(now_date_time, now_date,'外部依赖未完成，退出')
        exit()