...
def judge_the_task_group_run_func(task_group_dict):
    ...
    if will_start_task_group_flag:
        ...
        for line in file_list:
            ...
            if line_list[0] == now_date and now_date_time == line_list[1]:
        ...
        if running_flag:
            ...
            print(now_date, now_date_time, file=f)

            f.close()
            start_the_task_group_run_func(will_start_task_group_dict)

        # print('将会调用这个', will_start_task_group_dict)

        # 这里得判断是否在运行
        # sql_obj = oracle_run_sql_class(223)
        # sql_obj.insert_into_sql_func(now_date_time, 'R')
...
def start_the_task_group_run_func(will_start_task_group_dict):
    ...
    for line in file_list:
        ...
        if line_list[0] == now_date and line_list[1] == run_time:
            print(line, '这个该删除')
            continue
        else:
            f.write(line)
