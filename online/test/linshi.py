...
class oracle_run_sql_class(object):
    ...
    def search_sql_func(self, search_sql):
        try:
            ...
            print(e)
            print("Oracle connect is broken!!!", search_sql)
    ...
    def insert_into_sql_common_func(self, insert_sql):
        try:
            ...
            print(e)
            print("Oracle connect is broken!!!", insert_sql)
...
class Call_StoredProcedure_Class(object):
    ...
    def Call_StoredProcedure(self, StoredProcedure_Name, para_list):
        ...
        print(now_date_time, StoredProcedure_Name, "start")
        conn = cx_Oracle.connect(self.which_db)
        cur = conn.cursor()
        ...
        conn.close()
        now_date_time = (datetime.datetime.now()).strftime("[INFO] [%Y%m%d %H:%S]")
        print(now_date_time, StoredProcedure_Name, "ok")
        return res
...
def check_run_done(now_date):
    ...
    search_sql = "select DATA_DATE, RUN_STATUS from EDWPDC.EDWPDC_FINISH_FLAG where DATA_DATE = '%s' and RUN_STATUS ='0'" % (
    now_date)
...
def check_pre_job_and_call_SP(SP_name, SP_name2):
    while True:
        search_sql_obj = oracle_run_sql_class(226)
        search_sql = "select * from dw.dw_dependences_v WHERE data_date = '%s' and source_base = '%s' and run_status = '0'" % (
        now_date, SP_name2)
