...
class Call_StoredProcedure_Class(object):
    ...
    def Call_StoredProcedure(self, StoredProcedure_Name, para_list):
        print(StoredProcedure_Name, 'running...')
        time.sleep(3)
        print(StoredProcedure_Name, 'done!')
        # conn = cx_Oracle.connect(self.which_db)
        # cur = conn.cursor()
...
def call_SP(task_group_id, task_group_info_dict):
    ...
    while check_all_job_done(task_group_id, task_group_info_dict):
    # 内部依赖， 返回可以调用的job_id
    ...
    # 检查所有作业是否完成
    # check_all_job_done(task_group_id)
    ...
    for job_id in task_group_info_dict['job_id'].keys():
        ...
        if will_call_job_flag:
            if done_SP_set == {'None'}:
                ...
                a = all_done_SP_set - done_SP_set
                for line in a:
                    if not line in will_run_job_id_list:
                        will_run_job_id_list.append(line)

    print(will_run_job_id_list)
...
def check_all_job_done(task_group_id, task_group_info_dict):
    ...
    if done_SP_set == all_done_SP_set:
        ...
        print("作业组 %s 所有作业尚未完成" % (task_group_id))
        no_done_job_id = all_done_SP_set - done_SP_set
        print('未完成任务', no_done_job_id)
        return True
